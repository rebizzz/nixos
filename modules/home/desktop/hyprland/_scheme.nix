{lib, ...}: {
  # seed for current.lua, which caelestia-cli owns after first boot (not managed here)
  ...
  home.activation.hyprSchemeBootstrap = lib.hm.dag.entryAfter ["writeBoundary"] ''
    current="$HOME/.config/hypr/scheme/current.lua"
    default="$HOME/.config/hypr/scheme/default.lua"
    if [ ! -e "$current" ]; then
      run install -m644 "$default" "$current"
    fi
  '';
}
